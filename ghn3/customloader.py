...
def image_loader(dataset='imagenet', data_dir='./data/', test=True, im_size=32,
                 ...
                 seed=1111, load_train_anyway=False, n_shots=None, ddp=False,
                 transforms_train_val=None, verbose=True):
    """
    ...
    if dataset.lower() == 'imagenet':
        if transforms_train_val is None:
            transforms_train_val = transforms_imagenet(noise=noise, cifar_style=False,
                                                       im_size=im_size)
        ...
        if not test or load_train_anyway:
            train_data = ImageNetDataset(imagenet_dir, 'train', transform=train_transform,
                                         has_validation=not test)

        valid_data = ImageNetDataset(imagenet_dir, 'val', transform=valid_transform,
                                     has_validation=not test)
        ...
        if transforms_train_val is None:
            transforms_train_val = transforms_imagenet(noise=noise, cifar_style=False, im_size=64)
        train_transform, valid_transform = transforms_train_val

        if not test or load_train_anyway:
            train_data = TinyImageNetDataset(data_dir, split='train', transform=train_transform)

        valid_data = TinyImageNetDataset(data_dir, split='val', transform=valid_transform)
        shuffle_val = False
        n_classes = 200
        generator = None
    else:
        ...
        if transforms_train_val is None:
            transforms_train_val = transforms_cifar(cutout=cutout, cutout_length=cutout_length,
                                                    noise=noise, sz=im_size)
        train_transform, valid_transform = transforms_train_val

        if test:
            valid_data = eval(
                '{}(data_dir, train=False, download=True, transform=valid_transform)'.format(
                    dataset))
            if load_train_anyway:
                train_data = eval(
                    '{}(data_dir, train=True, download=True, transform=train_transform)'.format(
                        dataset))
            ...
            train_data = eval(
                '{}(data_dir, train=True, download=True, transform=train_transform)'.format(
                    dataset))
            valid_data = eval(
                '{}(data_dir, train=True, download=True, transform=valid_transform)'.format(
                    dataset))
...
def to_few_shot(dataset, n_shots=100):
    ...
    idx = sorted(torch.cat([torch.tensor(v) for k, v in
                            labels_dict.items()]))  # sort according to the original order in the full dataset

    dataset.data = [dataset.data[i] for i in idx] if isinstance(dataset.data, list) else \
    dataset.data[idx]

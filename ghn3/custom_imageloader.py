"""
Filename : custom_imageloader.py
Author : Archit
    ...
    if dataset.lower() == 'imagenet':
        ...
        if not test or load_train_anyway:
            ...
            if n_shots is not None:
                train_data = to_percent_shot_imagenet(train_data, n_shots)
...
def to_percent_shot_imagenet(dataset, percent):
    """
    Subsamples a percentage of training data per class for ImageNet.

    :param dataset: ImageNetDataset object
    :param percent: percentage of examples to keep per class (e.g. 1 for 1%)
    :return: subsampled dataset
    """
    assert 0 < percent <= 100, f"Invalid percent: {percent}. Must be in (0, 100]"
